...
BasicGame.Game1.prototype = {
	create: function () {
		...
		this.shift = this.add.audio('shift');
		this.damage = this.add.audio('damage');
		...
		if (this.isPlayer1 == 1){
			...
			this.range = 150;
			this.superR = 400;
			this.superDescript = this.add.text(25,772,"Onslaught: Player 2 loses 800+ health.", {font: "24px Times New Roman", fill: "black", align: "center"});
		}
		...
		this.no = true;
		this.faze = this.add.sprite(0,0,'faze');
		this.tweenF = this.add.tween(this.faze).to({alpha: 0},1000,"Linear",true);
		this.shift.play();
	},
	update: function (){
	...
	meleeAttack: function (pointer) {
		if (this.no == true){
		this.tween.pause();
		this.tween1 = this.add.tween(this.player2).to({alpha: 0.5},100,"Linear",true,0,-1);
    	this.tween1.yoyo(true, 0);
    	this.damage.play();
		this.no = false;
		...
		this.time.events.repeat(Phaser.Timer.QUARTER / 2,100,this.makeSound,this);
		//Player 2 health decreases with timer.
		this.TimerH = this.time.events.loop(Phaser.Timer.QUARTER * 0.0000000001,this.updateCounterH,this);
		}
	},
	...
	superAttack: function (pointer){
		...
		if (this.isPlayer1 == 1){
			...
			this.game.state.states['Game2'].health2 = this.subH;
			this.time.events.repeat(Phaser.Timer.QUARTER / 2,25,this.makeSound,this);
			this.TimerH = this.time.events.loop(Phaser.Timer.QUARTER * 0.0000000001,this.updateCounterH,this);
		}
		else if (this.isPlayer1 == 2){
			//Super meter empties.
			...
			this.game.state.states['Game2'].health2 = this.subH;
			this.time.events.repeat(Phaser.Timer.QUARTER / 2,125,this.makeSound,this);
			this.TimerH = this.time.events.loop(Phaser.Timer.QUARTER * 0.0000000001,this.updateCounterH,this);
		}
		else if (this.isPlayer1 == 3){
			//Super meter empties.
	...
	makeSound: function (pointer){
		this.attackEffect.play(null,null,0.5,false,true);
		this.attackEffect.volume = 0.1;
	},
